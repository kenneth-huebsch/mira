...
`dripr-production-debug` is a skill label, not an OpenClaw agent id.
...
investigation in the main session. If `sessions_spawn` returns an error, stop
immediately and tell Kenny the investigation could not be started, including the
exact spawn error; do not retry with extra optional fields and do not attempt the
debug investigation in the parent session. Do not poll in a loop; OpenClaw
announces completion. Use `/subagents` or task tools only for intervention or
